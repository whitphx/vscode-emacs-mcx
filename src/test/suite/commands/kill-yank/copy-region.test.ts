...
import { assertTextEqual, clearTextEditor, setupWorkspace, cleanUpWorkspace, tick } from "../../utils";
...
ABCDEFGHIJ`;
    ...
    test("mark-mode is disabled and selections are unset after copy region with multi cursor", async () => {
        ...
        assert.ok(activeTextEditor.selections[0].isEqual(
            new Range(new Position(0, 5), new Position(0, 5))));
        assert.ok(activeTextEditor.selections[1].isEqual(
            new Range(new Position(1, 5), new Position(1, 5))));
        assert.ok(activeTextEditor.selections[2].isEqual(

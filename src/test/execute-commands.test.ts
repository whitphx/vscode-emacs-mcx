...
suite("executeCommands", () => {
    test("executing commands sequentially", async () => {
        ...
        function registerCommand(commandName: string, callback: (...args: any[]) => void) {
            disposables.push(
                vscode.commands.registerCommand(commandName, callback),
            );
        }
        registerCommand("mockCommand1", () => new Promise((resolve) => setTimeout(() => {
            results.push(1);
            resolve();
        }, 300)));
        registerCommand("mockCommand2", () => new Promise((resolve) => setTimeout(() => {
            results.push(2);
            resolve();
        }, 200)));
        registerCommand("mockCommand3", () => new Promise((resolve) => setTimeout(() => {
            results.push(3);
            resolve();
        }, 100)));

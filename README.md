...
| `M-SPC`                                          | ✓               | Cycle spacing: first reduces whitespace to one space, then deletes all whitespace, then restores original (cycle-spacing). `alt+space` is ignored by VSCode ([issue](https://github.com/microsoft/vscode/issues/199984)), so this keybinding only works with other Meta key options (Escape, Ctrl+[, or Cmd on macOS).                                                                                                           |

...
| `C-x C-t`                                        |                 | Transpose lines (transpose-lines). Exchanges the current line with the previous line and moves the cursor to the beginning of the next line. When cursor is on the first line, it transposes the first and second lines. With positive prefix argument, repeats the transpose that many times. With negative prefix argument, transposes backward (swaps with the line below).                                                   |
